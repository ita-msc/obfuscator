    ...
    public:
        ...
        virtual bool runOnModule(Module &M) {
            ...
            while( gi != ge ) {
                ...
                DEBUG_WITH_TYPE(DEBUG_TYPE, dbgs() << __PRETTY_FUNCTION__ << ": Global variable " << gv->getName() << " is part of section " << section << '\n' );
...
//					) {
                if (
						...
						section != "llvm.metadata" &&
						section.find("__objc_methname") == std::string::npos &&
						gv->getType()->getElementType()->getTypeID() == 14
					) { //&& origName[0] == '.' && origName[1] == 's' && origName[2] == 't' && origName[3] == 'r' ) {

                    ++GlobalsEncoded;
                    DEBUG_WITH_TYPE(DEBUG_TYPE, dbgs() << __PRETTY_FUNCTION__ << "  - is constant." << '\n' );

                    if( gv->getType()->isIntOrIntVectorTy(8) )
                    	DEBUG_WITH_TYPE(DEBUG_TYPE, dbgs() << __PRETTY_FUNCTION__ << "  - is 8 bit-width" << '\n' );
...
//                        }
            ...
            for( GlobalVariable* gvToDel : toDelConstGlob ) {
                DEBUG_WITH_TYPE(DEBUG_TYPE, dbgs() << __PRETTY_FUNCTION__ << "  - " << gvToDel->getName() << " is being removed." << '\n' );
